...
namespace SeNA80
{
    public partial class SeNARun : Form
    {
        ...
        public static string[] bases = new string[125];
        public static bool inParallel = true;
        public static int ibases = 0;
        public static bool bShuttingDown = false;
        public static Color cOn = Color.FromArgb(0, 210, 00);
        public static Color cOnBack = Color.White;
        public static Color cOff = Color.Red;
        public static Color cOffBack = Color.Black;
        public static Color cGasOn = Color.Blue;
        public bool bCheckSmart = false;

        public SeNARun()
        {
            InitializeComponent();
            //ProtoListView.OwnerDraw = true;

            //delegates to handle out of focus color on the loop steps Listview control
            Color LoopStepsOriginalColor = LoopStepsListView.BackColor;
            Color LoopStepsOriginalForeColor = LoopStepsListView.ForeColor;
            LoopStepsListView.GotFocus += delegate
            {
                LoopStepsListView.HideSelection = true;
                LoopStepsListView.BackColor = LoopStepsOriginalColor;
            };

            LoopStepsListView.LostFocus += delegate
            {
                //make them all whitel
                for (int l = 0; l < LoopStepsListView.Items.Count; l++)
                {
                    LoopStepsListView.Items[l].BackColor = LoopStepsOriginalColor;
                    LoopStepsListView.Items[l].ForeColor = LoopStepsOriginalForeColor;
                }
                //now find the selected one and make it green
                LoopStepsListView.HideSelection = true;
                ...
                foreach (int index in indexes)
                {
                    ...
                    LoopStepsListView.Items[index].BackColor = cOn;  //System.Drawing.SystemColors.HighLight -- too dark;
                    LoopStepsListView.Items[index].ForeColor = Color.White;
                }

            };
        }
        private void ProtoListView_DrawItem(object sender, DrawListViewItemEventArgs e)
        {

            if (e.Item.Selected)
            {
                ProtoListView.FullRowSelect = false;

                e.Item.BackColor = cOn;  //System.Drawing.SystemColors.HighLight -- too dark;
        ...
        private void ProtoListView_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
        {
            ProtoListView.HideSelection = false;
            ...
            if (e.Item.Selected)
            {
                ...
                switch (ProtoListView.Columns[e.ColumnIndex].TextAlign)
                {
                ...
                TextRenderer.DrawText(e.Graphics, e.SubItem.Text, ProtoListView.Font, bounds, Color.White,
        ...
        private void ProtoListView_DrawColumnHeader(object sender, DrawListViewColumnHeaderEventArgs e)
        {
        ...
        private void SeNARun_Load(object sender, EventArgs e)
        {
            ...
            //reinitialize the bases array
            bases = new string[125];
            ibases = 0;
            ...
            if (ltrcode != globals.i12Ltr)
            {
                if (SeqNo == 9)
                {
                    for (int l = 1; l < lines.Length; l++)
                    {
                        ...
                        if (int.TryParse(parts[0], out sCheck))
                        {
        ...
        private void SeNARun_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (globals.bIsRunning || bShuttingDown)  //don't allow closing
            {
                MessageBox.Show("Closing of the Run Program Not Allowed \nWhile running or Terminating. \nYou must first stop the run or wait for termination to complete...",
        ...
        private void CheckAlarms()
        {
            ...
            if (globals.bAmPresAlarm)
            {
                ...
                double dPres = globals.dAmidPres;  //Now using a global variable that is logged during the processing of the streamed string.
                                                   //should resolve the Double.Parse issue experienced before
            ...
            if (globals.bReagPresAlarm)
            {
                //get the value from the screen
                double dPres = globals.dReagPres;
                //we will show a Message Box, this will "freeze" the tread until user responds
                // if the user responds O.K. the run will continue, if Cancel, the run will terminate
        ...
        private bool checkSameScale()
        {
            ...
            //now compare
            for (int c = (firstempty + 1); c < 10; c++)
            {

                switch (c)
                {
                    case 2:
                        if (runSequences[c - 1].Length > 0)
                        {
                            //    MessageBox.Show(cb_Col1Scale.Text, runSequences[c - 1]);
                            if (!cbval.Equals(cb_Col1Scale.Text))
                                return false;
                        }
                        break;
                    case 3:
                        if (runSequences[c - 1].Length > 0)
                        {
                            //  MessageBox.Show(cb_Col2Scale.Text, runSequences[c - 1]);
                    ...
                    case 4:
                        if (runSequences[c - 1].Length > 0)
                        {
                            //MessageBox.Show(cb_Col3Scale.Text, runSequences[c - 1]);
                    ...
                    case 5:
                        if (runSequences[c - 1].Length > 0)
                        {
                            // MessageBox.Show(cb_Col4Scale.Text, runSequences[c - 1]);
                    ...
                    case 6:
                        if (runSequences[c - 1].Length > 0)
                        {
                            //MessageBox.Show(cb_Col5Scale.Text, runSequences[c - 1]);
                    ...
                    case 7:
                        if (runSequences[c - 1].Length > 0)
                        {
                            // MessageBox.Show(cb_Col6Scale.Text, runSequences[c - 1]);
                    ...
                    case 8:
                        if (runSequences[c - 1].Length > 0)
                        {
                            // MessageBox.Show(cb_Col7Scale.Text, runSequences[c - 1]);
                    ...
                    case 9:
                        if (runSequences[c - 1].Length > 0)
                        {
                            // MessageBox.Show(cb_Col8Scale.Text, runSequences[c - 1]);
        ...
        private async void btn_Run_Click(object sender, EventArgs e)
        {
            ...
            if (bMonitoringUV && globals.bSaveBar)
                if (WindowAlreadyOpen.WindowOpen("Trityl Bar Chart Histogram"))
                    globals.sb.Close();

            //check to see if 4 oligos or less
            ...
            if (rb_Smart.Checked)  //make sure all scales are the same
            {
                ...
                if (!bCheckSmart)
                {
                    ...
                    for (int i = 0; i < runSequences.Length; i++)
                    {
                        if (runSequences[i].Length > 0)
                            count = count + 1;
                    }
                    if (count > 4)
                    {
                        if (MessageBox.Show("It is not recommended to use the Smart Mode for\nmore than 4 oligos at the same time\n\n" +
                            "Would you like to continue in Smart Mode?", "Too Many Sequences", MessageBoxButtons.YesNo, MessageBoxIcon.Hand) == DialogResult.No)
                        {
                            rb_Smart.Checked = false;
                            rb_StdDelivery.Checked = true;
                            bCheckSmart = false;
                        }
                    }
                    bCheckSmart = false;
            ...
            if (lbl_Start.Text.Length > 1 && !(lbl_Start.Text.Contains("none")))  //you don't have to have a startup protocol
            {
                if (globals.bProtocolsLoaded && globals.bSeqeuncesLoaded)
                {
                    ...
                    this.Text = "RiboBio8 Run - Running the Start Protocol ...";
            ...
            for (int OligosToMake = 0; OligosToMake < TotalOligos; OligosToMake++)
            {
                ...
                //find the average scale and then add 67% more when synthesizing multiple oligos
                //the repeat cycles will be extended to accomodate for the number of oligos.
                if (globals.noOligos > 1)
                    globals.avgScale = (globals.avgScale / globals.noOligos) * (globals.noOligos * 0.67);
                else
                    globals.avgScale = (globals.avgScale / globals.noOligos);

                Man_Controlcs.WriteStatus("Scale Reagents - ", "Using " + globals.avgScale.ToString("0.0") + " as the scaling factor ");
                ...
                {
                    //Set Program Text to Running...
                    this.Text = "RiboBio8 Run - Running the Prep Protocol ...";
                    ...
                    if (bRunProcessing)
                    {
                        ...
                        //clear the looping list view
                        if (LoopStepsListView.Items.Count > 0)
                            LoopStepsListView.Items.Clear();
                        ...
                        if (MixPSPO && rb_Parallel.Checked)
                        {
                        ...
                        for (globals.iCycle = StartCycle; globals.iCycle < MaxCycles; globals.iCycle++)
                        {
                            ...
                            //fill the list view
                            LoopStepsListView.Items.Clear();
                            FillTheLoopListView(whichlines);
                            ...
                            Man_Controlcs.WriteStatus("Run Program", "Deblocking...");
                            ...
                            this.Text = "RiboBio8 Run - Performing Synthesis Cycle " + (globals.iCycle + 1).ToString() + " of " + MaxCycles.ToString() + ".....";
                            SafeSetStatus("Performing Synthesis Cycle " + (globals.iCycle + 1).ToString() + " of " + MaxCycles.ToString() + ".....");
                            Man_Controlcs.WriteStatus("Run Program", "Performing Synthesis Cycle " + (globals.iCycle + 1).ToString() + " of " + MaxCycles.ToString() + ".....");
                            ...
                            for (int i = 0; i < LoopSteps; i++)
                            {
                                ...
                                if (i < LoopStepsListView.Items.Count)
                                {
                                    LoopStepsListView.Items[i].Selected = true;
                                    LoopStepsListView.FullRowSelect = true;

                                    LoopStepsListView.Focus();

                                }

                                int p = 0;

                                if (LoopStepsListView.SelectedIndices.Count > 0)
                                    p = LoopStepsListView.SelectedIndices[0];

                                if (p > 3 && p < LoopStepsListView.Items.Count - 3)
                                    scrollto = p + 3;
                                else
                                    scrollto = LoopStepsListView.Items.Count - 1;

                                if (p > 3)
                                    LoopStepsListView.Items[scrollto].EnsureVisible();

                                //also select it in the list box
                                ...
                                else if (!parts[0].Contains("Coupling"))
                                {
                                    ...
                                    if (parts[0].Contains("Oxi"))
                                    {
                                        ...
                                        Man_Controlcs.WriteStatus("Run Program", "Oxidation...");
                                        globals.bOxidizing = true;
                                    }
                                    if (parts[0].Contains("Thio"))
                                    {
                                        this.lbl_curBase.Text = "Post Coupling --  Thiolation";
                                        Man_Controlcs.WriteStatus("Run Program", "Thiolation...");
                                    ...
                                    if (parts[0].Contains("Mix"))
                                    {
                                        ...
                                        Man_Controlcs.WriteStatus("Run Program", "Ox/Thio...");
                                        globals.bOxidizing = true;
                                    ...
                                    Man_Controlcs.WriteStatus("Run Program", "Capping...");
                                ...
                                else  // Coupling Protocol
                                {
                                    ...
                                    if (bMonitoringUV && rb_Sequenntial.Checked && globals.bSaveBar)    // !WindowAlreadyOpen.WindowOpen("Trityl Bar Chart Histogram"))
                                        Menu_UV_Bar.PerformClick();
                                    else if (!bMonitoringUV && bAutoShowChart && globals.bSaveBar)
                                        Menu_UV_Bar.PerformClick();
                                    ...
                                    for (int seq = 0; seq <= 8; seq++)//one at a time get the sequence
                                    {
                                        ...
                                        if (curSeq.Length > 0 && curSeq.Length > globals.iCycle)
                                        {
                                            ...
                                            globals.dAmiditeCF = Properties.Settings.Default.ReagentFC;
                                            if (rb_Smart.Checked && bRunProcessing && !bTerminateEndofCycle && !bTerminateEndofStep)
                                            {
                                                if (sameBase.Length > 1)
                                                {
                                                    ...
                                                    Man_Controlcs.WriteStatus("Run", "The scaling factor being used is -  " + globals.dAmiditeCF.ToString("0.0"));
                                        ...
                                        if (rb_Smart.Checked && bRunProcessing && !bTerminateEndofCycle && !bTerminateEndofStep)
                                        {
                                            ...
                                            if (!lastcycle) //if is is the last cycle don't worry about this...
                                            {
                                                ...
                                                if (sameBase[0] == -1 || lastcycle) { seq = 8; }

                                                //reset the array
                                                if (seq < 8)//chenged here 
                                                    sameBase = new int[9 - seq];
                    ...
                    lb_loop.Items.Clear();
                    LoopStepsListView.Items.Clear();  //and the listview
                    this.lbl_curBase.Text = "";

                    //Final Protocol = Run Post Synthesis Protocol
                    time = String.Format("{0:F}", DateTime.Now);
                    ...
                    if (bRunProcessing && !bTerminateEndofCycle && !bTerminateEndofStep)
                    {
                        //if we are monitoring UV Trityl and the Post Protocol contains Deblock then turn monitor on for last step
                        if (this.lbl_Post.Text.Contains("Deb") || this.lbl_Post.Text.Contains("Off"))
                        {
                            //turn the cells on
                            for (int u = 0; u < runSequences.Length; u++)
                            {
                                if (runSequences[u].Length > 0)
                                {
                                    switch (u)
                            ...
                            Array.Clear(globals.iUVTritylData, 0, globals.iUVTritylData.Length);
                            globals.DeblockMonitor = true;
                            //globals.iCycle = globals.iCycle+1;
                        }

                        FillTheList(this.lbl_Post.Text);

                        //Set Program Text to Running...
                        this.Text = "RiboBio8 Run - Running the Post Run Protocol ...";
                    ...
                    //process UV Trityl Data if Monitored
                    if (globals.DeblockMonitor)
                    {
                        ProcessUVTrityl();
                        globals.DeblockMonitor = false;
                        ...
                        //since this is the last cycle, if they want save the image
                        if (globals.tritylreport == 2 || globals.tritylreport == 0)  //save the picture
                        {
                            if (globals.bSaveHist && globals.bSaveBar)
                                barchartcontrol.SavePict();
                        }
        ...
        private void rb_Smart_CheckedChanged(object sender, EventArgs e)
        {
            if (!bCheckSmart)
            {
                bCheckSmart = true;
        ...
        private void pnl_btns_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void Menu_RestartTrityl_Click(object sender, EventArgs e)
        {
            if (globals.bUVTrityl) //there is a monitor
            {
                ...
                //start the stream
                Main_Form.StartSensorData(1);  //just restart the stream
            }
        }
